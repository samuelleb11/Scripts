...
$Global:var_Sys_RegistryKeyName = "AltaroBackup"
...
$Global:var_Sys_AltaroEventList = @()
$Global:var_Sys_AltaroFilteredEventList = @()
...
# GET THE ALTARO EVENT LIST AFTER THE LAST SCAN DATE EXCLUDING KEYWORD IN REGISTRY
$Global:var_Sys_AltaroEventList = Get-EventLog -LogName Application -Source 'Altaro VM Backup' -After $Date
foreach ($AltaroEvent in $Global:var_Sys_AltaroEventList)
{
    if (!(Check-Keywords -KeywordList $Global:var_Sys_List_ExcludeKeyword -ValueToCheck $AltaroEvent.Message))
    {
        $Global:var_Sys_AltaroFilteredEventList += $AltaroEvent
    }
}
...
foreach ($Event in $Global:var_Sys_AltaroFilteredEventList)
{
